<template>
  <div
    v-if="event_data != null && event_data.MenuTimeEnd != null"
    class="w-full max-w-screen-xl py-2 mx-auto mb-5 text-center rounded-lg bg-secondary bg-opacity-20"
  >
    <p class="mb-2 text-sm text-basic_gray">限時活動剩餘時間</p>
...
export default {
  ...
  methods: {
    SetTimer() {
      ...
      setInterval(() => {
        if (this.event_data != null && this.event_data.MenuTimeEnd != null) {
          // 設置倒數計時: 結束時間 - 當前時間
          // 當前時間
          let time = new Date();
